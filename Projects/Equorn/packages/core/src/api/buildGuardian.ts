/**
 * BuildGuardian API
 * Modern API for generating projects from seed files with improved error handling and result tracking
 */
...
// Stand-alone implementation to avoid dependency issues
// This makes the README example work without relying on other parts of the codebase
...
export async function buildGuardian(options: BuildGuardianOptions): Promise<GenerationResult> {
  ...
  console.log('[DEBUG buildGuardian] path.join:', outputDir, target);
...
async function generateGodotProject(seedData: any, outputDir: string, verbose: boolean): Promise<string[]> {
  const files: string[] = [];
...
config/name="${seedData.world?.name || 'Generated World'}"
...
glow_enabled = true`;
  ...
  await fs.writeFile(projectPath, projectConfig.trim());
  files.push(projectPath);

  // Generate main scene with unique ID
...
offset_right = 432.0
offset_bottom = 82.0
text = "${seedData.world?.name || 'Generated World'} - Generated by Equorn v4.4.1"
...
`;
  ...
  await fs.writeFile(scenePath, mainScene.trim());
...
glow_enabled = true`;
  ...
  const readmeContent = `# ${seedData.world?.name || 'Entity in Environment'}
    
This is a generated Godot 4.4.1 project for the ${seedData.world?.name || 'Entity'} myth.
